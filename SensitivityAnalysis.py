...
def plot_entropy_evolution_comparison(entropy_x_ind, entropy_y_ind, entropy_z_ind,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('plots', f'{num_conditions}_rossler_entropy_x_comparison.png'), dpi=300)
    ...
    plt.tick_params(labelsize=fs-5)
    plt.tight_layout()
    plt.savefig(os.path.join('plots', f'{num_conditions}_rossler_entropy_y_comparison.png'), dpi=300)
    ...
    plt.tick_params(labelsize=fs-5)
    plt.tight_layout()
    plt.savefig(os.path.join('plots', f'{num_conditions}_rossler_entropy_z_comparison.png'), dpi=300)
    ...
    plt.tick_params(labelsize=fs-5)
    plt.tight_layout()
    plt.savefig(os.path.join('plots', f'{num_conditions}_rossler_entropy_xyz_joint.png'), dpi=300)
...
def main():
    ...
    # Parameters
    num_conditions_array = [1000, 1500, 2000, 2500]
    time_final = 70
    dt = 1e-2
    time_steps = int(time_final / dt)

    # Arrays to store results for plotting later
    all_time_points = []  # Store time points for all initial conditions
    all_entropy_xyz_joint = []  # Store entropy_xyz_joint for all initial conditions
    
    
    for num_conditions in num_conditions_array:
        print("Starting Execution for condition with value " + str(num_conditions))
        # Generate initial conditions
        initial_conditions = gaussian_initial_condition_generator(num_conditions)
        print("Gaussian initial conditions generated for condition with value " + str(num_conditions))
        ...
        print("PMFs and entropies for gaussian initial conditions calculated for condition with value " + str(num_conditions))
        ...
        # Save the data for plotting after the loop finishes
        time_points = np.linspace(0, time_final, time_steps)  # Generate time points
        all_time_points.append(time_points)
        all_entropy_xyz_joint.append(entropy_xyz_joint)
        print("Analysis complete! Figures saved for condition with value " + str(num_conditions))
        
    plt.figure(figsize=(10, 6))
    for time_points, entropy_xyz_joint, num_conditions in zip(all_time_points, all_entropy_xyz_joint, num_conditions_array):
        # Plot entropy for each initial condition set
        plt.plot(time_points, entropy_xyz_joint, label=f'{num_conditions} Initial Conditions')
    # Customize the plot
    plt.xlabel(r'$t$', fontsize=14)
    plt.ylabel(r'$H\left[XYZ(t)\right]$', fontsize=14)
    plt.legend()
    plt.title(f'Joint Entropy Evolution for Different Initial Conditions (Final Time = {time_final})')
    plt.tight_layout()
    plt.savefig(os.path.join('plots', f'joint_entropy_evolution_{time_final}.png'), dpi=500)
    print("All analyses complete! All figures saved.")
